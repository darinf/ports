...
#error "Not supported yet."
  ...
  parent_message_pipe = CreateParentMessagePipe(pipe_token);
...
void MultiprocessTestHelper::ChildSetup() {
  ...
  std::string pipe_token = base::CommandLine::ForCurrentProcess()
      ->GetSwitchValueASCII(kMojoPrimordialPipeToken);
  CHECK(!pipe_token.empty());

  SetParentPipeHandle(
      PlatformChannelPair::PassClientHandleFromParentProcess(
          *base::CommandLine::ForCurrentProcess()));
  client_message_pipe = CreateChildMessagePipe(pipe_token);
}

// static
ScopedMessagePipeHandle MultiprocessTestHelper::client_message_pipe;
