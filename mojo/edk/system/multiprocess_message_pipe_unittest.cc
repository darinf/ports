...
DEFINE_TEST_CLIENT_WITH_PIPE(EchoEcho, MultiprocessMessagePipeTest, h) {
  ...
  for (;; rv = (rv + 1) % 100) {
    ...
    MojoResult result = MojoWait(h, MOJO_HANDLE_SIGNAL_READABLE,
...
TEST_F(MultiprocessMessagePipeTest, MAYBE_Basic) {
  ...
  ON_PIPE(h)
    ...
    HandleSignalsState hss;
    ASSERT_EQ(MOJO_RESULT_OK,
              MojoWait(h, MOJO_HANDLE_SIGNAL_READABLE, MOJO_DEADLINE_INDEFINITE,
                       &hss));
...
TEST_F(MultiprocessMessagePipeTest, MAYBE_QueueMessages) {
  ...
  END_CHILD_WITH_EXIT_CODE(static_cast<int>(kNumMessages % 100))
}
...
DEFINE_TEST_CLIENT_WITH_PIPE(CheckSharedBuffer, MultiprocessMessagePipeTest,
  ...
  uint32_t num_handles = MOJO_ARRAYSIZE(handles);  // Maximum number to receive
...
TEST_F(MultiprocessMessagePipeTest, MAYBE_SharedBufferPassing) {
  RUN_WITH_CHILD(CheckSharedBuffer)
  ON_PIPE(h)
    ...
    ASSERT_EQ(MOJO_RESULT_OK,
              MojoWriteMessage(h, &go1[0], static_cast<uint32_t>(go1.size()),
                               &handles[0], MOJO_ARRAYSIZE(handles),
                               MOJO_WRITE_MESSAGE_FLAG_NONE));
...
class MultiprocessMessagePipeTestWithPipeCount
    : public test::MultiprocessTestBase,
...
TEST_P(MultiprocessMessagePipeTestWithPipeCount, PlatformHandlePassing) {
  ...
  RUN_WITH_CHILD(CheckPlatformHandleFile)
  ON_PIPE(h)
...
INSTANTIATE_TEST_CASE_P(PipeCount,
                        ...
                        // TODO: Enable 140-handle test once POSIX supports
                        // passing a large number of handles.
...
DEFINE_TEST_CLIENT_WITH_PIPE(CheckMessagePipe, MultiprocessMessagePipeTest, h) {
  ...
  CHECK_EQ(MojoReadMessage(h, nullptr, nullptr, &handles[0], &num_handles,
                           MOJO_READ_MESSAGE_FLAG_NONE),
           MOJO_RESULT_OK);
  CHECK_EQ(num_handles, 1u);
...
TEST_F(MultiprocessMessagePipeTest, MAYBE_MessagePipePassing) {
  RUN_WITH_CHILD(CheckMessagePipe)
  ON_PIPE(h)
    MojoCreateSharedBufferOptions options;
    options.struct_size = sizeof(options);
    options.flags = MOJO_CREATE_SHARED_BUFFER_OPTIONS_FLAG_NONE;
...
TEST_F(MultiprocessMessagePipeTest, MAYBE_DataPipeConsumer) {
  RUN_WITH_CHILD(DataPipeConsumer)
  ON_PIPE(h)
    MojoCreateSharedBufferOptions options;
    options.struct_size = sizeof(options);
    options.flags = MOJO_CREATE_SHARED_BUFFER_OPTIONS_FLAG_NONE;
